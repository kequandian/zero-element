    ...
    format: function format(string) {
      ...
      if (typeof string === 'string') {
        ...
        keyList && keyList.forEach(function (key) {
          if (key.indexOf('[') > -1) {
            string = string.replace(key, dataPool.getLocationSearch()[key.replace(/\[|\]/g, '')]);
          } else if (key.indexOf('(') > -1) {
            string = string.replace(key, getDeepValue(dataPool.record, key.replace(/\(|\)/g, '')));
          } else if (key.indexOf('{') > -1) {
            string = string.replace(key, getDeepValue(model.state.formData, key.replace(/\{|\}/g, '')));
          } else if (key.indexOf('<') > -1) {
            string = string.replace(key, getDeepValue(data, key.replace(/\<|\>/g, '')));
          }
        });
      } else {
